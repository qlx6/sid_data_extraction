...
rm <- read.xlsx("Zimbabwe_RM_2021.xlsx", sheet = "RM Functional Resp", rows = 9:114, cols = 2:16)
#ou <- readxl::read_excel("Botswana_RM_2021.xlsx", sheet = "RM Functional Resp", range = "B3:B3",col_names = FALSE)[[1]]
...
rm_aux <- read_csv(file = "rm_aux_titles.csv",
                   trim_ws = TRUE)
...
rm_9 <- rm_8 %>% mutate(
  Func_Elem_Disagg_Sub = case_when(
    ...
    UUID == "RMUID_034" ~ "Consumables",
    UUID == "RMUID_035" ~ "Consumables",
    UUID == "RMUID_036" ~ "Consumables",
    UUID == "RMUID_037" ~ "Consumables",
...
openxlsx::write.xlsx(rm_12, file = paste("RM_",ou, "_", period,"_",date, "_Time_0500hrs.xlsx", sep = ""), keepNA = T, asTable = TRUE)
